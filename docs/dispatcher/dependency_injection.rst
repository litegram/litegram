...
For example, to get :class:`litegram.fsm.context.FSMContext` we do it like that:
...
Litegram provides several ways to complement / modify contextual data.

The first and easiest way is to simply specify the named arguments in
:class:`litegram.dispatcher.dispatcher.Dispatcher` initialization, polling start methods
or :class:`litegram.webhook.litestar_server.SimpleRequestHandler` initialization if you use webhooks.
...
:class:`litegram.dispatcher.dispatcher.Dispatcher`'s workflow data also can be supplemented
...
The default middleware data typed dict can be found in :class:`litegram.dispatcher.middlewares.data.MiddlewareData`.

In case when you have extended the context data, you can use the :class:`litegram.dispatcher.middlewares.data.MiddlewareData` as a base class and specify the type hints for the new fields.
...
.. autoclass:: litegram.dispatcher.middlewares.data.MiddlewareData
    :members:
    :undoc-members:
    :member-order: bysource


.. autoclass:: litegram.dispatcher.middlewares.data.I18nData
    :members:
    :undoc-members:
    :member-order: bysource
