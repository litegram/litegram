...
from litegram.client.telegram import TelegramAPIServer
from litegram.methods import GetFile, GetMe
...
class TestBot:
    def test_init(self):
        bot = Bot("42:TEST")
        assert isinstance(bot.session, HttpxSession)
        assert bot.id == 42
    ...
    async def test_emit(self, bot: Bot):
        ...
        with patch(
            "litegram.client.session.httpx.HttpxSession.make_request",
            new_callable=AsyncMock,
    ...
    @pytest.mark.anyio
    async def test_close(self, session: HttpxSession):
        bot = Bot("42:TEST", session=session)
        await session.create_client()

        with patch("litegram.client.session.httpx.HttpxSession.close", new_callable=AsyncMock) as mocked_close:
    ...
    async def test_context_manager(self, close: bool):
        with patch(
            target="litegram.client.session.httpx.HttpxSession.close",
            new_callable=AsyncMock,
        ) as mocked_close:
    ...
    @pytest.mark.anyio
    async def test_download_file(self, httpx_mock: HTTPXMock, file_path):
    ...
    async def test_download(self, mocked_bot: MockedBot):
        mocked_bot.add_result_for(GetFile, ok=True, result=File(file_id="file id", file_unique_id="file id"))
        mocked_bot.add_result_for(GetFile, ok=True, result=File(file_id="file id", file_unique_id="file id"))
        ...
        with pytest.raises(TypeError):
            await mocked_bot.download([PhotoSize(file_id="file id", file_unique_id="file id", width=123, height=123)])
