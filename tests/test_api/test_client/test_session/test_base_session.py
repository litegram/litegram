...
from litegram.client.default import Default, DefaultBotProperties
from litegram.client.session.base import BaseSession

# Removed TelegramType import
from litegram.client.telegram import PRODUCTION, TelegramAPIServer
from litegram.enums import ChatType, ParseMode, TopicIconColor
from litegram.exceptions import (
    ClientDecodeError,
    RestartingTelegram,
    TelegramAPIError,
    TelegramBadRequest,
...
from litegram.methods import DeleteMessage, GetMe, TelegramMethod
from litegram.types import UNSET_PARSE_MODE, LinkPreviewOptions, User
from litegram.types.base import UNSET_DISABLE_WEB_PAGE_PREVIEW, UNSET_PROTECT_CONTENT
...
class TestBaseSession:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                datetime.datetime(year=2017, month=5, day=17, hour=4, minute=11, second=42, tzinfo=utc),
                "1494994302",
            ],
            [
    ...
    def test_prepare_value_defaults_replace(self):
        ...
        assert bot.session.prepare_value(Default("link_preview_is_disabled"), bot=bot, files={}) == "true"
