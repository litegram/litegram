...
from litegram.exceptions import CallbackAnswerException
from litegram.methods import AnswerCallbackQuery
from litegram.types import CallbackQuery, User
from litegram.utils.callback_answer import CallbackAnswer, CallbackAnswerMiddleware
...
class TestCallbackAnswerMiddleware:
    ...
    async def test_call(self, properties, expected_stack):
        ...
        with (
            patch(
                "litegram.utils.callback_answer.CallbackAnswerMiddleware.construct_callback_answer",
                new_callable=MagicMock,
                side_effect=lambda **kwargs: CallbackAnswer(**{"answered": False, **properties}),
            ),
            patch(
                "litegram.utils.callback_answer.CallbackAnswerMiddleware.answer",
                new=answer,
            ),
        ):
