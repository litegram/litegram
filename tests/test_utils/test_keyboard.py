...
from litegram.filters.callback_data import CallbackData
...
from litegram.utils.keyboard import (
...
class TestKeyboardBuilder:
    ...
    def test_validate_button(self):
        ...
        assert builder._validate_button(button=InlineKeyboardButton(text="test", callback_data="callback"))
    ...
    def test_validate_buttons(self):
        ...
        assert builder._validate_button(InlineKeyboardButton(text="test", callback_data="callback"))

    def test_validate_row(self):
        builder = ReplyKeyboardBuilder()

        with pytest.raises(ValueError):
            assert builder._validate_row(row=(KeyboardButton(text=f"test {index}") for index in range(builder.max_width + 5)))

        with pytest.raises(ValueError):
            assert builder._validate_row(row=[KeyboardButton(text=f"test {index}") for index in range(builder.max_width + 5)])

        for count in range(11):
            assert builder._validate_row(row=[KeyboardButton(text=f"test {index}") for index in range(count)])
