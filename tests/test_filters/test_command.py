...
class TestCommandFilter:
    ...
    async def test_command_magic_result(self, bot: MockedBot):
        ...
        assert "args" in result  # type: ignore
        assert result["args"] == "42"  # type: ignore

    @pytest.mark.anyio
    async def test_empty_mention_is_none(self, bot: MockedBot):
        # Fixed https://github.com/litegram/litegram/issues/1013:
        ...
        assert "command" in result  # type: ignore
        command_obj: CommandObject = result["command"]  # type: ignore
...
class TestCommandStart:
    def test_str(self):
        cmd = CommandStart()
        assert str(cmd) == "CommandStart(ignore_case=False, ignore_mention=False, deep_link=False, deep_link_encoded=False)"
