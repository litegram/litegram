...
from litegram import Dispatcher, F, Router
from litegram.dispatcher.event.bases import NextMiddlewareType
from litegram.enums import UpdateType
from litegram.exceptions import SceneException
from litegram.filters import Command, StateFilter
from litegram.fsm.context import FSMContext
from litegram.fsm.scene import (
    ActionContainer,
    After,
    HandlerContainer,
    HistoryManager,
...
class TestObserverDecorator:
    ...
    def test_wrap_handler(self):
        ...
        assert hasattr(wrapped, "__litegram_handler__")
        assert isinstance(wrapped.__litegram_handler__, list)
        assert len(wrapped.__litegram_handler__) == 1

        wrapped2 = decorator(handler)

        assert len(wrapped2.__litegram_handler__) == 2
    ...
    def test_wrap_action(self):
        ...
        wrapped = decorator(handler)
        assert wrapped is not None
        assert not hasattr(wrapped, "__litegram_handler__")
        assert hasattr(wrapped, "__litegram_action__")

        assert isinstance(wrapped.__litegram_action__, dict)
        assert len(wrapped.__litegram_action__) == 1
        assert SceneAction.enter in wrapped.__litegram_action__
        assert "test" in wrapped.__litegram_action__[SceneAction.enter]  # type: ignore
...
class TestActionContainer:
    ...
    async def test_action_container_execute_enter(self):
        ...
        action_container = ActionContainer("Test Name", (F.text,), SceneAction.enter, "Test Target")
...
class TestSceneHandlerWrapper:
    ...
    async def test_scene_handler_wrapper_call_with_after(self):
        ...
        with patch("litegram.fsm.scene.ActionContainer.execute", new_callable=AsyncMock) as after_mock:
            result = await scene_handler_wrapper(event_update_mock, **kwargs)
...
class TestHistoryManager:
    ...
    async def test_history_manager_push(self):
        state = FSMContext(storage=MemoryStorage(), key=StorageKey(bot_id=42, chat_id=-42, user_id=42))
...
class TestScene:
    def test_scene_subclass_initialisation(self):
        ...
        class SubScene(
            ...
            general_handler = on.message(F.text.casefold() == "test", after=After.goto("sub_state"))
    ...
    def test_scene_add_to_router_scene_with_callback_query_without_state(self):
        ...
        assert StateFilter(MyScene.__scene_config__.state) not in router.observers["callback_query"].handlers[0].filters  # type: ignore
...
class TestSceneRegistry:
    ...
    def test_scene_registry_get_scene_if_scene_state_is_not_str(self):
        ...
        with pytest.raises(SceneException, match="Scene must be a subclass of Scene, State or a string"):
...
class TestSceneInheritance:
    ...
    def test_inherit_actions(self):
        ...
        parent_enter_action = ParentScene.__scene_config__.actions[SceneAction.enter][UpdateType.MESSAGE]
        child_enter_action = ChildScene.__scene_config__.actions[SceneAction.enter][UpdateType.MESSAGE]
