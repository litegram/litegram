...
from litegram.dispatcher.event.handler import CallableObject, FilterObject, HandlerObject
...
class TestCallableObject:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(callback1, {"foo": 42, "spam": True, "baz": "fuz"}, {"foo": 42, "baz": "fuz"}),
            pytest.param(
                callback2,
            ...
            pytest.param(TestFilter(), {"foo": 42, "spam": True, "baz": "fuz"}, {"foo": 42, "baz": "fuz"}),
            pytest.param(SyncCallable(), {"foo": 42, "spam": True, "baz": "fuz"}, {"foo": 42, "baz": "fuz"}),
        ],
    )
    def test_prepare_kwargs(self, callback: Callable, kwargs: dict[str, Any], result: dict[str, Any]):
        obj = CallableObject(callback)
...
class TestHandlerObject:
    ...
    async def test_check_with_dict_result(self):
        handler = HandlerObject(
            simple_handler,
            [FilterObject(functools.partial(lambda value, index: {f"test{index}": "ok"}, index=item)) for item in range(3)],
        )
        result, data = await handler.check(42, foo=True)
        assert result
    ...
    async def test_check_partial_rejected(self):
        handler = HandlerObject(simple_handler, [FilterObject(lambda value: True), FilterObject(lambda value: False)])
        result, data = await handler.check(42, foo=True)
        assert not result
