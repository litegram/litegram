...
from litegram.dispatcher.event.telegram import TelegramEventObserver
from litegram.dispatcher.router import Router
from litegram.exceptions import UnsupportedKeywordArgument
...
class TestTelegramEventObserver:
    def test_register(self):
        ...
        assert observer.handlers[1].filters[0].callback == f  # type: ignore

        observer.register(my_handler, MyFilter1(test="PASS"))
        assert isinstance(observer.handlers[2], HandlerObject)
        assert any(isinstance(item.callback, MyFilter1) for item in observer.handlers[2].filters)  # type: ignore
        ...
        callbacks = [filter_.callback for filter_ in observer.handlers[3].filters]  # type: ignore
    ...
    async def test_trigger_right_context_in_handlers(self):
        ...
        observer.register(pipe_handler, mix_unnecessary_data, handler)  # {"a": 1} should not be in result
    ...
    def test_register_global_filters(self):
        ...
        router.message.filter(my_filter)
        assert len(router.message._handler.filters) == 1  # type: ignore
        assert router.message._handler.filters[0].callback is my_filter  # type: ignore
