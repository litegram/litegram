...
from litegram.dispatcher.event.bases import UNHANDLED, SkipHandler
from litegram.dispatcher.router import Router
from litegram.methods import GetMe, GetUpdates, SendMessage, TelegramMethod
...
class TestDispatcher:
    ...
    @pytest.mark.anyio
    async def test_storage_property(self, dispatcher: Dispatcher):
        assert dispatcher.storage is dispatcher.fsm.storage

    @pytest.mark.anyio
    async def test_parent_router(self, dispatcher: Dispatcher):
    ...
    async def test_listen_updates(self, bot: MockedBot):
        ...
        bot.add_result_for(GetUpdates, ok=True, result=[Update(update_id=update_id) for update_id in range(42)])
        index = 0
    ...
    async def test_listen_update_with_error(self, bot: MockedBot):
        ...
        with patch(
            "litegram.utils.backoff.Backoff.asleep",
    ...
    async def test_listen_update(
        ...
        async def my_handler(event: Any, **kwargs: Any):
            assert event.model_dump(exclude_defaults=True) == getattr(update, event_type).model_dump(exclude_defaults=True)
            if has_chat:
        ...
        assert result["event_update"].model_dump(exclude_defaults=True) == update.model_dump(exclude_defaults=True)
    ...
    async def test_process_update_call_request(self, bot: MockedBot):
        ...
        with patch(
            "litegram.dispatcher.dispatcher.Dispatcher.silent_call_request",
            new_callable=AsyncMock,
    ...
    async def test_process_update_exception(self, bot: MockedBot, caplog):
        ...
        assert any("Cause exception while process update" in rec.message for rec in caplog.records)
    ...
    async def test_polling(self, bot: MockedBot, as_task: bool):
        ...
        with (
            ...
            patch("litegram.dispatcher.dispatcher.Dispatcher._listen_updates") as patched_listen_updates,
        ):
    ...
    async def test_polling_with_semaphore(
        ...
        with (
            patch("litegram.dispatcher.dispatcher.Dispatcher._process_update", new_callable=AsyncMock),
            patch("litegram.dispatcher.dispatcher.Dispatcher._listen_updates") as patched_listen_updates,
    ...
    async def test_process_with_semaphore_exception(self):
        ...
        with pytest.raises(Exception, match="Test exception"):
            await dispatcher._process_with_semaphore(mock_handle_update_with_exception(), semaphore)
    ...
    async def test_concurrent_updates_limit(self, bot: MockedBot):
        ...
        with (
            patch(
                "litegram.dispatcher.dispatcher.Dispatcher._process_update",
            ...
            polling_task = asyncio.create_task(
                dispatcher._polling(bot=bot, handle_as_tasks=True, tasks_concurrency_limit=tasks_concurrency_limit)
            )
    ...
    async def test_start_polling(self, bot: MockedBot):
        ...
        with pytest.raises(ValueError, match="At least one bot instance is required to start polling"):
            await dispatcher.start_polling()
        with pytest.raises(
            ValueError,
            match="Keyword argument 'bot' is not acceptable, the bot instance should be passed as positional argument",
        ):
        ...
        with (
            patch("litegram.dispatcher.dispatcher.Dispatcher._process_update", new_callable=AsyncMock) as mocked_process_update,
            patch("litegram.dispatcher.router.Router.emit_startup", new_callable=AsyncMock) as mocked_emit_startup,
            patch("litegram.dispatcher.router.Router.emit_shutdown", new_callable=AsyncMock) as mocked_emit_shutdown,
            patch("litegram.dispatcher.dispatcher.Dispatcher._listen_updates") as patched_listen_updates,
        ):
            patched_listen_updates.return_value = _mock_updates()
    ...
    def test_run_polling(self, bot: MockedBot):
        ...
        with patch(
            "litegram.dispatcher.dispatcher.Dispatcher.start_polling",
    ...
    def test_specify_updates_calculation(self):
        ...
        assert sorted(useful_updates4) == sorted(["message", "callback_query", "poll", "edited_message"])
