    ...
    def resolve_event_context(cls, event: Update) -> EventContext:
        ...
        if event.message:
            return EventContext(
                ...
                thread_id=(event.message.message_thread_id if event.message.is_topic_message else None),
            )
        if event.edited_message:
            return EventContext(
                chat=event.edited_message.chat,
                user=event.edited_message.from_user,
                thread_id=(event.edited_message.message_thread_id if event.edited_message.is_topic_message else None),
            )
        ...
        if event.business_message:
            return EventContext(
                ...
                thread_id=(event.business_message.message_thread_id if event.business_message.is_topic_message else None),
        ...
        if event.edited_business_message:
            return EventContext(
                ...
                thread_id=(
                    event.edited_business_message.message_thread_id if event.edited_business_message.is_topic_message else None
                ),
